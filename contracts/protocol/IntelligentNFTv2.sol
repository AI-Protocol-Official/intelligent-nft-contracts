...
contract IntelligentNFTv2 is IntelligentNFTv2Spec, AccessControl, ERC165 {
	 ...
	 */
	constructor(address _ali) AccessControl(msg.sender, 0) {
	...
	function mint(
		...
		// emit an event
		emit Minted(msg.sender, owner, recordId, aliValue, personalityContract, personalityId, targetContract, targetId);
	}
	...
	function mintBatch(
		...
		for (uint96 i = 0; i < n; i++) {
			...
			// verify if AI Personality is already transferred to iNFT
			require(
				ERC721(personalityContract).ownerOf(personalityId + i) == address(this),
